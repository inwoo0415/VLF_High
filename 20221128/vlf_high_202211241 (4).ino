...
void loop() // 계속 반복 실행
{
  ...
  L_distance = L_Sonar_sensor(); // 왼쪽 거리값은 왼쪽 초음파 센서 값
  R_distance = R_Sonar_sensor(); // 오른쪽 거리값은 오른쪽 초음파 센서 값 
  F_distance = F_Sonar_sensor(); // 전방 거리값은 전방 초음파 센서 값
  Serial.print("L:"); Serial.print( L_distance);  // 왼쪽 초음파 센서 출력값 L : 
  Serial.print("   F:"); Serial.print( F_distance); // 전방 초음파 센서 출력값 F :
  Serial.print("   R:"); Serial.println( R_distance); // 오른쪽 초음파 센서 출력값 R :
 
  measure_width = L_distance + robot_width + R_distance; // measure_width 변수는 왼쪽 초음파 센서의 거리 값 + 차 폭 + 오른쪽 초음파 센서 거리 값  

  if( (measure_width>= (mazor_width-  mazor_width_tolerance) ) && (measure_width <= (mazor_width +  mazor_width_tolerance) ))// measure_width 변수가 도로폭에서 변수를 뺀값과 더한 값 사이에 위치한다면 
  {
     ...
     {  
        ...
        Serial.println("flag_mazor_1 set"); //"flag_mazor_1 set", ln은 줄바꿈을 의미
     }
  }
  else
  {
